...
fs.readFile('booking.txt',await function (err, filedata) {
    ...
    bookings.forEach((value, key) => {
        console.log('Booking: ' + key + ' guest: ' + value[0] + ' Checkin: ' + value[1] + ' Checkout: ' + value[2])
    })
...
create_booking = (room_id, guestr_id, checkin_date, checkout_date) => {
    bookings.set(bookings.size+1, [room_id, guest_id, checkin_date, checkout_date])
    saveBooking()
    return `[${room_id} ${guest_id} ${checkin_date} ${checkout_date}]`
}
